...
export function LoginForm() {
  ...
  useEffect(() => {
    const checkSession = async () => {
      ...
      if (!isAllowedLoginRole(session.user.role)) {
        setError("Your account is not authorized to access this admin area.")
        return
      }
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      if (!isAllowedLoginRole(session.user.role)) {
        setError("Only admin, editor, or superadmin accounts may sign in here.")
        setIsSubmitting(false)
        return
      }
  ...
  const handleGoogleSignIn = async () => {
    ...
    try {
      ...
      // The client handles the redirect automatically
    ...
    <AuthCard>
      <div className="space-y-8">
        <AuthHeader
          ...
          subtitle="Use email/password or Google sign-in for admin/editor access."
        />
